...
// --- EPUB parsing (Node-side, no DOM/window — epub.js is browser-only) ---

function parseXml(text: string): Document {
  return new DOMParser().parseFromString(text, 'application/xml') as unknown as Document
}

// Find elements by local name, tolerant of namespace prefixes (e.g. dc:title).
function elsByLocal(node: { getElementsByTagName: (n: string) => ArrayLike<Element> }, local: string): Element[] {
  const all = node.getElementsByTagName('*')
  const out: Element[] = []
  for (let i = 0; i < all.length; i++) {
    const el = all[i]
    const tn = el.tagName || ''
    if ((el as { localName?: string }).localName === local || tn === local || tn.endsWith(':' + local)) {
      out.push(el)
    }
  }
  return out
...
// Resolve an href (relative to `dir`) into a normalized zip path using '/'.
...
function extractBodyText(doc: Document): string {
  const body = elsByLocal(doc as unknown as { getElementsByTagName: (n: string) => ArrayLike<Element> }, 'body')[0] || doc
  let text = ''
...
async function getOpfContext(zip: JSZip): Promise<OpfContext> {
  ...
  const opfPath = elsByLocal(container as any, 'rootfile')[0]?.getAttribute('full-path')
...
async function parseEpubMetadata(filePath: string): Promise<{
  title: string
  author: string
  ...
  const title = xmlText(elsByLocal(opf as any, 'title')) || path.basename(filePath, '.epub')
  const author = xmlText(elsByLocal(opf as any, 'creator'))

  // Build manifest: id -> { href, type, properties }
  const manifest = new Map<string, { href: string; type: string; props: string }>()
  for (const item of elsByLocal(opf as any, 'item')) {
  ...
  // OPF-relative href, regardless of which sub-file (ncx/nav) it came from.
  ...
  // TOC: prefer NCX (EPUB2), then nav.xhtml (EPUB3), then spine order.
  let toc: { label: string; href: string }[] = []
  const spineEl = elsByLocal(opf as any, 'spine')[0]
  ...
  if (ncxEntry) {
    ...
    if (ncxFile) {
      ...
      toc = elsByLocal(ncx as any, 'navPoint').map((np) => ({
  ...
  if (toc.length === 0) {
    ...
    if (navEntry) {
      ...
      if (navFile) {
        ...
        const navs = elsByLocal(navDoc as any, 'nav')
        const tocNav =
          navs.find((n) =>
  ...
  if (toc.length === 0) {
    const spineHrefs = elsByLocal(opf as any, 'itemref')
  ...
  try {
    ...
    if (!coverHref) {
      const coverMeta = elsByLocal(opf as any, 'meta').find((m) => m.getAttribute('name') === 'cover')
    ...
    if (coverHref) {
      ...
      if (coverFile) {
        const ext = path.extname(coverZipPath) || '.jpg'
        coverPath = path.join(getCoversDir(), `${uuidv4()}${ext}`)
        fs.writeFileSync(coverPath, await coverFile.async('nodebuffer'))
      }
    }
  } catch {
    // cover optional
...
async function parsePdfMetadata(filePath: string): Promise<{
  title: string
  author: string
  coverPath: string | null
  chapters: Omit<Chapter, 'id' | 'bookId'>[]
...
export async function importBook(): Promise<Book | null> {
  ...
  const format: BookFormat = ext === '.epub' ? 'epub' : 'pdf'

  const bookId = uuidv4()
  const destPath = path.join(getBooksDir(), `${bookId}${ext}`)
...
export function getFileData(bookId: string): Uint8Array {
  const book = getBook(bookId)
  if (!book || !fs.existsSync(book.filePath)) {
    throw new Error('书籍文件不存在')
  }
  return new Uint8Array(fs.readFileSync(book.filePath))
}
...
// Returns a chapter's body HTML with images inlined as data URLs, for direct
// rendering in the renderer (we render EPUB ourselves — epub.js is unreliable
// with in-memory archives in Electron).
export async function getEpubChapterHtml(bookId: string, href: string): Promise<string> {
  ...
  try {
    ...
    const body = elsByLocal(doc as any, 'body')[0]
    if (!body) return ''

    // Strip scripts.
    for (const s of elsByLocal(body as any, 'script')) (s as any).parentNode?.removeChild(s)

    // Inline images (html <img src> and svg <image href/xlink:href>).
    const imgs = [...elsByLocal(body as any, 'img'), ...elsByLocal(body as any, 'image')]
    ...
    let html = ''
    for (let c = (body as any).firstChild; c; c = c.nextSibling) {
